...
public class NSRActivityIntent extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (ActivityRecognitionResult.hasResult(intent)) {
            nsr.getTracerManager().getActivityTracer().stopTrace();
            ...
            try {
                ...
                if (candidate != null && !candidate.equals(nsr.getTracerManager().getActivityTracer().getLastActivity()) &&
                        maxConfidence >= nsr.getDataManager().getConfigurationRepository().getConf().getJSONObject("activity").getInt("confidence")) {
                    JSONObject payload = new JSONObject();
                    payload.put("type", candidate);
                    payload.put("confidence", maxConfidence);
                    nsr.getProcessorManager().getEventProcessor().crunchEvent("activity", payload);
                    nsr.getTracerManager().getActivityTracer().setLastActivity(candidate);
                    nsr.getTracerManager().getLocationTracer().setStillLocation("still".equals(candidate));
                }
            } catch (Exception e) {
